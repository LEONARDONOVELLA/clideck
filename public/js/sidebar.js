// Collapsible sidebar: toggle button at the top of the left nav rail (desktop).
...
export function initSidebarToggle() {
  ...
  btn.className = 'rail-btn w-9 h-9 flex items-center justify-center rounded-lg text-slate-500 hover:text-slate-300 hover:bg-slate-800/50 transition-colors';
  btn.innerHTML = '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="9" y1="4" x2="9" y2="20"/></svg>';
  btn.title = 'Hide sidebar';
  rail.insertBefore(btn, rail.firstChild);

  btn.addEventListener('click', (e) => {
    e.stopPropagation(); // keep nav.js and the auto-expand handler out of this
    collapsed = !collapsed;
    apply();
  });

  // Clicking any other nav-rail item while collapsed re-opens the sidebar
  rail.addEventListener('click', (e) => {
    if (e.target.closest('#rail-browser, #rail-browser-detached')) return; // browser toggles don't need the sidebar
